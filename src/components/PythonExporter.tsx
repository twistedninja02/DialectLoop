...
import { Copy, Check, Download, FileCode, CheckCircle2, FileJson, Archive } from 'lucide-react';
...
export default function PythonExporter({ activeBatch }: PythonExporterProps) {
...
`;
  ...
  const downloadCleanedJson = () => {
    ...
    // Build highly polished clean JSON payload matching archival targets inpredictions/
    const archivePayload = {
      archive_metadata: {
        framework: "DialectLoop v1.1",
        exported_at: new Date().toISOString(),
        batch_id: activeBatch.batch_id,
        batch_name: activeBatch.name,
        status: activeBatch.status,
        final_iteration_count: activeBatch.iterations.length,
        error_rate_threshold_tau: activeBatch.error_rate_threshold,
        converged: activeBatch.status === 'completed',
        reproducibility_guidelines: "To replicate, store this JSON under predictions/ and run pytest or python dialectloop_multi_agent.py"
      },
      segments: activeBatch.segments.map(seg => ({
        segment_id: seg.segment_id,
        district: seg.district,
        duration: seg.duration,
        speaker_id: seg.speaker_id,
        original_transcript: seg.transcript,
        final_transcript: activeBatch.confirmed_corrections[seg.segment_id] || seg.transcript,
        corrections_applied: !!activeBatch.confirmed_corrections[seg.segment_id]
      })),
      iteration_history: activeBatch.iterations.map(iter => ({
        iteration_index: iter.iteration_index,
        batch_error_rate: iter.batch_error_rate,
        top_patterns: iter.top_patterns,
        recommended_action: iter.recommended_action,
        self_summary: iter.self_summary,
        critic_decisions: Object.values(iter.critic_decisions).map(dec => ({
          segment_id: dec.segment_id,
          consensus_flag: dec.consensus_flag,
          uncertainty: dec.uncertainty,
          escalated: dec.escalated,
          resolution_reasoning: dec.resolution_reasoning,
          auditor_errors: dec.auditor_errors.map(err => ({
            error_type: err.error_type,
            error_token: err.error_token,
      ...
      human_gate_feedback: {
        total_corrections_made: Object.keys(activeBatch.confirmed_corrections).length,
        corrections: activeBatch.confirmed_corrections
      }
    };
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `predictions_${activeBatch.batch_id}_clean.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="space-y-6">
      {/* Active Predictions Archival Card */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <span className="text-[10px] font-mono text-purple-600 uppercase font-bold tracking-widest block">Active Batch Archival</span>
              <h3 className="font-display font-bold text-slate-900 text-sm mt-0.5">Predictions &amp; Iteration History Export</h3>
              <p className="text-xs text-slate-500 mt-1 max-w-xl">
                Export predictions and the multi-agent consensus log as a standardized, cleaned JSON document. Suitable for preservation in the <code className="bg-slate-100 px-1 py-0.5 rounded font-mono font-semibold text-rose-600 text-[10px]">predictions/</code> directory to satisfy archival and replication standards.
              </p>
            </div>
          </div>
          ...
          {activeBatch ? (
            <button
              ...
              className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-purple-600 text-white text-xs font-bold hover:bg-purple-700 hover:shadow-md transition duration-150 cursor-pointer self-start sm:self-center whitespace-nowrap"
            >
              <Archive className="w-4 h-4" />
              Export Cleaned JSON
            </button>
          ) : (
            <div className="text-xs text-slate-400 font-mono flex items-center gap-1.5 self-start sm:self-center">
              <span>No active batch loaded</span>
            </div>
          )}
        </div>

        {activeBatch && (
          <div className="mt-5 pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-4 text-[11px] font-mono">
            <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-150">
              <span className="text-slate-400 block font-bold uppercase text-[9px]">Target Batch</span>
              <span className="text-slate-700 font-semibold truncate block mt-0.5">{activeBatch.name}</span>
            </div>
            <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-150">
              <span className="text-slate-400 block font-bold uppercase text-[9px]">Processed Status</span>
              <span className={`font-semibold capitalize mt-0.5 block ${activeBatch.status === 'completed' ? 'text-emerald-600' : 'text-amber-600'}`}>
                {activeBatch.status === 'needs_review' ? 'Gate #1 Pending' : activeBatch.status}
              </span>
            </div>
            <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-150">
              <span className="text-slate-400 block font-bold uppercase text-[9px]">Completed Iterations</span>
              <span className="text-indigo-600 font-bold block mt-0.5">{activeBatch.iterations.length} run(s)</span>
            </div>
          </div>
        )}
      </div>

      {/* Main Python Implementation Exporter Container */}
